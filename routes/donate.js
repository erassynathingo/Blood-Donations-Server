...
router.post('/', auth.authenticate, (req, res, next) => {
  ...
  controller.create(req).then(doc => {
    ...
    logger.log(moduleName+' Updating Error: ', error).error(error)
    let json = errorHandler.resolve(error, config.response.status_codes.NOT_UPDATED, moduleName+' could not be Created: ' + error.messsage)
